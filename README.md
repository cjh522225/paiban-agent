...
> 说明：排班系统本体为团队/课程项目（多人协作，已在校园实际部署使用）；
> 本仓库为作者个人维护的 Agent 升级版本，新增了 `agent-service` 与前端悬浮球助手等模块。
...
- **工具编排**：Spring AI `ChatClient` + `@Tool`，19 个查询工具 + 26 个写操作工具（请假/换班/多排审批、排班生成与手工调班、消息、用户、空闲时间等）
